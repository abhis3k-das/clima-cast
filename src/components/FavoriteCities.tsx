...
function FavoriteCityTablet({id , name ,lat , lon , onRemove} : FavoriteCityTabletProps){
    ...
    return <div className="relative flex min-w-[250px] cursor-pointer items-center gap-3 rounded-lg border bg-card p-4 pr-8 shadow-sm transition-all hover:shadow-md" tabIndex={0} role="button" onClick={()=> navigate(`city/${name}?lat=${lat}&lon=${lon}`)}>
        <Button onClick={(e)=> {
            e.stopPropagation();
            onRemove(id);
            toast.error(`Removed ${name} from favorites.`)
        }} variant={"ghost"} size={"icon"} className="absolute right-1 top-1 h-6 w-6 rounded-full p-0 hover:text-destructive group-hover:opacity-100">
            <X className="h-4 w-4"/>
        </Button>
